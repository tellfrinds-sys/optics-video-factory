# -*- coding: utf-8 -*-
"""يبني صفحتي فهرس الدروس (لمسارين: الممارسين والجمهور) من Supabase وينشرهما على الموقع.
   يُستدعى تلقائيًا بعد كل إنتاج ناجح، أو يدويًا:  python3 build_lessons_page.py
   primary_audience_uid: 1/2/4 = محتوى مهني (ممارسين) — 3 = محتوى تثقيفي مبسّط (جمهور)
"""
...
OUT_PRO = SITE / "lessons.html"
OUT_PUB = SITE / "lessons-public.html"
...
def _fetch(audience_filter):
    return _get("lessons?select=lesson_uid,lesson_code,title_ar,chapter_uid,"
        ...
        "&" + audience_filter + "&order=sort_order")
...
def _build_track(rows, doors, units, chaps, out_path, title, subtitle, nav_label_self, nav_href_other, nav_label_other, open_door):
    total = len(rows)
    ...
    for d in doors:
        ...
        sections.append(
            ...
            '<span class="lx-count">%d/%d فيديو</span></summary>%s</details>' % (
                " open" if d["door_code"] == open_door else "", _esc(d["door_code"]), _esc(d["title_ar"]),
                d_ready, d_total, "\n".join(d_inner)))

    if not sections:
        sections = ['<div class="lx-empty">المحتوى في هذا المسار قيد الإعداد — تابعونا قريبًا.</div>']

    page = (TEMPLATE
        .replace("{{TITLE}}", _esc(title))
        .replace("{{SUBTITLE}}", _esc(subtitle))
        .replace("{{TOTAL}}", str(total)).replace("{{READY}}", str(ready))
        .replace("{{SECTIONS}}", "\n".join(sections))
        .replace("{{NAV_SELF}}", _esc(nav_label_self))
        .replace("{{NAV_OTHER_HREF}}", nav_href_other)
        .replace("{{NAV_OTHER}}", _esc(nav_label_other))
        .replace("{{UPDATED}}", __import__("time").strftime("%Y-%m-%d %H:%M")))
    out_path.write_text(page, encoding="utf-8")
    print("wrote", out_path, len(page), "bytes;", ready, "/", total, "videos")
    return total, ready
...
def build():
    ...
    pro_rows = _fetch("primary_audience_uid=neq.3")
    pub_rows = _fetch("primary_audience_uid=eq.3")

    pt, pr = _build_track(
        pro_rows, doors, units, chaps, OUT_PRO,
        "فهرس الدروس والفيديوهات — مسار الممارسين",
        "المنهج المهني الكامل لأكاديمية بوابة البصريات — شرح معمّق ومفصّل لممارسي وطلاب البصريات.",
        "مسار الممارسين (الحالي)", "lessons-public.html", "مسار الجمهور ←", "B01")
    ut, ur = _build_track(
        pub_rows, doors, units, chaps, OUT_PUB,
        "فهرس الدروس والفيديوهات — مسار الجمهور",
        "شروحات مبسّطة وتثقيفية لغير المتخصصين — بدون تفاصيل مهنية أو تعليمات تشغيل.",
        "مسار الجمهور (الحالي)", "lessons.html", "→ مسار الممارسين", None)

    try:
        subprocess.run(["git", "-C", str(SITE), "add", "lessons.html", "lessons-public.html"], check=False, timeout=30)
        subprocess.run(["git", "-C", str(SITE), "-c", "user.email=bot@opticsgate.online",
                        "-c", "user.name=OpticsGate Bot", "commit", "-m",
                        "تحديث فهرسي الدروس (ممارسين %d/%d، جمهور %d/%d)" % (pr, pt, ur, ut)], check=False, timeout=30)
...
<title>{{TITLE}} | بوابة البصريات</title>
 ...
 *{box-sizing:border-box} body{margin:0}
 ...
 .lx-track{position:sticky;top:92px;z-index:19;background:rgba(6,11,24,.9);backdrop-filter:blur(10px);border-bottom:1px solid rgba(148,178,214,.1)}
 .lx-track div{max-width:1100px;margin:0 auto;padding:10px 22px;display:flex;gap:10px}
 .lx-tbtn{padding:7px 16px;border-radius:999px;font-size:13px;font-weight:700}
 .lx-tbtn.self{background:rgba(45,212,191,.16);border:1px solid rgba(45,212,191,.45);color:#8ff7e6}
 .lx-tbtn.other{background:rgba(148,178,214,.08);border:1px solid rgba(148,178,214,.22);color:#aebcd4}
 ...
 .lx-empty{text-align:center;color:#8fa0bc;padding:60px 20px;border:1px dashed rgba(148,178,214,.25);border-radius:16px;margin-top:20px}
...
<div class="lx-track"><div>
 <span class="lx-tbtn self">{{NAV_SELF}}</span>
 <a class="lx-tbtn other" href="{{NAV_OTHER_HREF}}">{{NAV_OTHER}}</a>
</div></div>
<div class="lx-wrap">
 <div class="lx-hero">
  <h1>{{TITLE}}</h1>
