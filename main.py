...
def stability_generate_image(prompt: str) -> bytes:
    ...
    full_prompt = (
        "professional medical educational illustration, clean flat vector style, "
        "no text, no watermark, calm teal and navy color palette, anatomical diagram: "
        + prompt
    )
    fields = {
        "prompt": full_prompt,
...
def render_frame_ai(scene: dict[str, Any], path: Path, index: int, total: int) -> None:
    ...
    try:
        ...
        return render_frame(scene, path, index, total)
    image = bg_image
