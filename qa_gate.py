...
def gemini_audio_review(audio_path: Path, scenes: list, lesson_goal: str = "") -> dict:
    ...
    sysmsg = (
        ...
        "افحص: (1) اللهجة: الأساس عامية مصرية، ومسموح تمامًا بألفاظ أو روابط فصيحة لو كانت "
        "أدق أو أوضح للمحتوى (مثال معتمد: السطح الأمامي أفضل من السطح القدامي). لا تُسجّل "
        "أي كلمة فصيحة كمخالفة لمجرد كونها فصيحة -- المخالفة الحقيقية الوحيدة هي الشذوذ: كلمة "
        "أو تعبير غريب/مقحم يكسر انسياب الكلام ويوقف الأذن، سواء كان فصحى ثقيلة بلا داعٍ أو عامية "
        "غريبة عن الموضوع. (2) مخارج الحروف والمصطلحات صحيحة "
        "(دايوبتر، الصلبة بفتح الصاد، الأسماء الإنجليزية). قواعد نطق دقيقة يجب فحصها: (أ) «القرنية» وأمثالها من المصطلحات العلمية البحتة تُنطق بالقاف قافًا واضحة — نطقها همزة (الأرنية) خطأ. (ب) «سُمك» (بمعنى السماكة/الثخانة) تُنطق بضمّ السين؛ نطقها بفتح السين (سَمك) خطأ يغيّر المعنى. (ج) الجيم في الكلمات الأجنبية/العلمية مثل «أكسجين» و«فسيولوجيا» تُنطق معطَّشة (كحرف j الإنجليزي/چ) لا جيمًا مصرية /g/. (د) بقيّة الحروف تُنطق باللهجة المصرية الطبيعية. (3) مواضع التوقف طبيعية — لا وقفات طويلة بين "
...
def run_qa(video_uid: int, scenes: list[dict]) -> dict:
    ...
    hard_fail = any(not c["ok"] for c in checks if c["check"] in (
        ...
        "تفريغ_الصوت", "مطابقة_النص", "خلو_من_الممنوع",
        "مراجعة_سمعية_Gemini"))  # خلو_من_الفصحى بقى معلوماتي فقط (قاعدة 2026-09-17)
    return {
